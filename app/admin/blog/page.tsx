  ...
  return (
    <div className="p-4 sm:p-6 md:p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-[22px] font-bold">{t("post.blogTitle")}</h1>
          <p className="text-[13px] text-muted mt-1">
            SEO maqolalar. <Link href="/admin/categories" className="text-accent hover:underline">{t("post.tabCategories")}</Link> ·{" "}
            <Link href="/admin/tags" className="text-accent hover:underline">{t("post.tabTags")}</Link>
          </p>
        </div>
      ...
      <div className="space-y-3">
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2 text-[12px] mb-1 flex-wrap">
                  ...
                  <span className={`px-2 py-0.5 rounded-full border ${STATUS_LABEL[p.status].className}`}>{t(STATUS_LABEL[p.status].labelKey as any)}</span>
        ...
        {posts.length === 0 && (
          <div className="rounded-xl glass-card p-8 text-center text-[13px] text-[#5b6f85]">{t("post.noArticles")}</div>
        )}
      </div>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-5">
          <form onSubmit={save} className="w-full max-w-2xl rounded-2xl border border-subtle bg-panel max-h-[90vh] overflow-y-auto p-6">
            ...
            <label className="block text-[12px] text-muted mb-1.5">{t("post.fTags")}</label>
            ...
            <details className="mb-3">
              ...
              <textarea rows={2} placeholder={t("post.phSeoDesc")} value={form.seo_description} onChange={(e) => setForm({ ...form, seo_description: e.target.value })} className={`${inputCls} mb-2`} />
              <input placeholder={t("post.phKeywords")} value={form.seo_keywords} onChange={(e) => setForm({ ...form, seo_keywords: e.target.value })} className={inputCls} />
            </details>
