...
export default function PromoManagePage() {
  ...
  const load = async () => {
    try {
      ...
      if (!res.ok) { toast.error("Yuklashda xatolik"); return; }
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      toast.success("Aksiya sozlamasi saqlandi ✅");
      load();
    } finally {
      setSaving(false);
    }
  ...
  const setWinner = async (customerId: string, segment: "cash" | "online", place: number) => {
    ...
    if (!res.ok) { toast.error("Bajarilmadi"); return; }
    toast.success(`${place}-o'rin g'olib belgilandi`);
    load();
  };
  const removeWinner = async (segment: string, place: number) => {
    ...
    if (res.ok) { toast.success("Olib tashlandi"); load(); }
  };
  ...
  const RankTable = ({ rows, segment, title, boost }: { rows: Row[]; segment: "cash" | "online"; title: string; boost: boolean }) => (
    <div className="rounded-2xl border border-white/10 overflow-hidden">
      ...
      {rows.length === 0 ? (
        <div className="px-4 py-6 text-center text-white/30 text-sm">Bu ro'yxatда mijoz yo'q.</div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead className="text-white/40 text-xs border-b border-white/10">
              <tr>
                ...
                <th className="text-right px-3 py-2">Faollik</th>
                {boost && <th className="text-right px-3 py-2">×{multN}</th>}
                <th className="text-center px-3 py-2">G'olib (o'rin)</th>
              </tr>
            </thead>
            <tbody>
  ...
  return (
    <div className="p-4 md:p-6 max-w-5xl mx-auto">
      <div className="flex items-center gap-2.5 mb-5">
        ...
        <div>
          <h1 className="text-lg font-semibold text-white">Sovrinli aksiya</h1>
          <p className="text-xs text-white/40">Muddat, sovrinlar, naqd/onlayn reyting va g'oliblar</p>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-4 mb-6">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-[14px] font-semibold text-white">Aksiya sozlamasi</h2>
          <button
          ...
          >
            {enabled ? "Aksiya yoniq" : "Aksiya o'chiq"}
          </button>
        </div>
        <div className="grid md:grid-cols-2 gap-3 mb-3">
          <div><span className="block text-[11px] text-white/40 mb-1">Boshlanish</span><input type="datetime-local" className={inp} value={start} onChange={(e) => setStart(e.target.value)} /></div>
          <div><span className="block text-[11px] text-white/40 mb-1">Tugash</span><input type="datetime-local" className={inp} value={end} onChange={(e) => setEnd(e.target.value)} /></div>
          <div><span className="block text-[11px] text-white/40 mb-1">Naqd koeffitsiyenti (masalan 1.5)</span><input type="number" step="0.1" className={inp} value={mult} onChange={(e) => setMult(e.target.value)} /></div>
          <div><span className="block text-[11px] text-white/40 mb-1">Sovrinlar (har qatorда bittadan)</span><textarea rows={2} className={inp} value={prizes} onChange={(e) => setPrizes(e.target.value)} placeholder="1-o'rin: Mashina&#10;2-o'rin: Telefon" /></div>
        </div>
        <button onClick={saveSettings} disabled={saving} className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-[13px] font-medium bg-accent/20 text-white hover:bg-accent/30 disabled:opacity-50">
          {saving ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />} Saqlash
        </button>
      </div>
      ...
      {winners.length > 0 && (
        <div className="rounded-2xl border border-[#F4C76A]/25 bg-[#F4C76A]/[0.04] p-4 mb-6">
          <h2 className="text-[13px] font-semibold text-[#F4C76A] flex items-center gap-1.5 mb-2.5"><Crown size={15} /> Tasdiqlangan g'oliblar</h2>
            ...
            {winners.map((w) => (
              <div key={w.id} className="flex items-center justify-between gap-2 text-[12.5px] bg-white/[0.03] rounded-lg px-3 py-2">
                <span className="text-white">
                  <span className="text-white/40">{w.segment === "cash" ? "Naqd" : "Onlayn"} · {w.place}-o'rin:</span> {w.customers?.full_name || w.customers?.phone || "—"}
                </span>
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        <RankTable rows={cash} segment="cash" title="Naqd mijozlar" boost={true} />
        <RankTable rows={online} segment="online" title="Onlayn mijozlar" boost={false} />
      </div>
    </div>
  );
}
