...
export default function MediaLibraryPage() {
  ...
  const handleFiles = async (files: FileList | File[]) => {
    ...
    for (const file of fileArray) {
      try {
        ...
        setError((prev) => (prev ? `${prev}; ` : "") + `${file.name}: ${e.message ?? t("med.uploadErr")}`);
      }
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8">
      <h1 className="text-[22px] font-bold mb-1">{t("med.title")}</h1>
      <p className="text-[13px] text-muted mb-6">{t("med.sub")}</p>
      ...
      >
        {uploading ? (
          <div className="flex flex-col items-center gap-2">
            ...
            <span className="text-[13px] text-muted">{t("med.uploading")} {uploadProgress.done}/{uploadProgress.total}</span>
          </div>
        ) : (
          <div className="flex flex-col items-center gap-2">
            <Upload size={22} className="text-[#5b6f85]" />
            <span className="text-[13px] text-muted">{t("med.dropHint")}</span>
          </div>
        )}
        <input
              ...
              <div className="absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 transition flex flex-col justify-between p-2">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => copyUrl(item)} className="p-1 rounded bg-white/10 hover:bg-white/20" aria-label={t("med.copyUrl")}>
                      {copiedId === item.id ? <Check size={11} className="text-[#4ADE80]" /> : <Copy size={11} className="text-white" />}
                    </button>
                    <button onClick={() => remove(item.id)} className="p-1 rounded bg-white/10 hover:bg-[#FF3B5C]/30" aria-label={t("med.del")}>
