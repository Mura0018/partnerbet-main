...
export function Card({
  ...
  return <As className={`rounded-2xl ${premium ? "card-premium" : "border border-subtle bg-white/[0.02]"} ${className}`} style={style}>{children}</As>;
}
...
export function EmptyState({ icon, message, hint }: { icon?: React.ReactNode; message: string; hint?: string }) {
  return (
    <div className="rounded-2xl border border-subtle bg-white/[0.02] p-10 text-center">
